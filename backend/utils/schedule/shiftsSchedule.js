...
const checkShiftAround = (monthShifts, day, type = "") => {
    ...
    const notOFFAround =
        (yesterday && yesterday[1] === "OFF" && twoDaysAgo && twoDaysAgo[1] !== "OFF")
        || (tomorrow && tomorrow[1] === "OFF" && twoDaysLater && twoDaysLater[1] !== "OFF")
    switch (type) {
        case "N":
            condition = notNAround
            break;
        case "OFF":
            condition = notOFFAround
            break;
        default:
...
const finalSchedule = (requestedShifts, allMonthShifts, stdPersonCount, year, month) => {
    ...
    let nullCounts = 0
    allMonthShifts.forEach(user => {
        user.monthShifts.forEach(dayShift => {
            if (!dayShift) nullCounts++
        })
    })
    console.log(`nullCounts -> `, nullCounts)
...
const primarySchedule = (allMonthShifts, stdPersonCount, year, month) => {
    const { totalDays, holidayMap, requestedDays } = daysAndRequests(year, month, allMonthShifts)
                ...
                if (!shiftDay) {
                    if (nextInsert === "N") {
                        const insertNResult = insertShiftController("N", user, { day, year, month },
                            { remainPersonCount, monthMax: checkUser.monthMax }, "&&")
                        if (insertNResult) {
                            nextInsert = "E"
                            continue;
                        } else nextInsert = "E"
...
const finalSchedule = (requestedShifts, allMonthShifts, stdPersonCount, year, month) => {
    const { totalDays, holidayMap, requestedDays } = daysAndRequests(year, month, requestedShifts)
